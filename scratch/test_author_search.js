...
['aladin_search_debug.html', 'temp_aladin_search.html', 'temp_search.html', 'temp_mobile_search.html'].forEach(f => {
  ...
  // Let's find any links containing AuthorSearch or whose href contains SearchWord or Author
  const links = Array.from(doc.querySelectorAll('a'));
  const authorLinks = links.filter(l => {
    const href = l.getAttribute('href') || '';
    return href.includes('AuthorSearch') || href.includes('Author') || href.includes('search');
  });
