...
export function LocationSettingModal({ currentLocation, onClose, onConfirm }: LocationSettingModalProps) {
  ...
  const locations = [
    ...
    "경기 과천시", "경기 광명시", "경기 광주시", "경기 구리시", "경기 군포시",
    "경기 김포시", "경기 남양주시", "경기 동두천시", "경기 부천시",
    "경기 성남시 분당구", "경기 성남시 수정구", "경기 성남시 중원구",
    "경기 수원시 권선구", "경기 수원시 영통구", "경기 수원시 장안구", "경기 수원시 팔달구",
    "경기 시흥시", "경기 안산시 단원구", "경기 안산시 상록구",
    "경기 안성시", "경기 안양시 동안구", "경기 안양시 만안구",
    "경기 양주시", "경기 양평군", "경기 여주시", "경기 연천군", "경기 오산시",
    "경기 용인시 기흥구", "경기 용인시 수지구", "경기 용인시 처인구",
    "경기 의왕시", "경기 의정부시", "경기 이천시", "경기 파주시", "경기 평택시",
    "경기 포천시", "경기 하남시", "경기 화성시", "경기 가평군",
    ...
    "충북 음성군", "충북 제천시", "충북 증평군", "충북 진천군", "충북 청주시 상당구",
    "충북 청주시 서원구", "충북 청주시 청원구", "충북 청주시 흥덕구", "충북 충주시",
    
    // 충청남도
    "충남 계룡시", "충남 공주시", "충남 금산군", "충남 논산시", "충남 당진시",
    "충남 보령시", "충남 부여군", "충남 서산시", "충남 서천군", "충남 아산시",
    "충남 예산군", "충남 천안시 동남구", "충남 천안시 서북구", "충남 청양군",
    "충남 태안군", "충남 홍성군",
    
    // 전북특별자치도
    "전북 고창군", "전북 군산시", "전북 김제시", "전북 남원시", "전북 무주군",
    "전북 부안군", "전북 순창군", "전북 완주군", "전북 익산시", "전북 임실군",
    "전북 장수군", "전북 전주시 덕진구", "전북 전주시 완산구", "전북 정읍시", "전북 진안군",
    ...
    "경북 예천군", "경북 울진군", "경북 의성군", "경북 청도군", "경북 청송군",
    "경북 칠곡군", "경북 포항시 남구", "경북 포항시 북구",
    
    // 경상남도
    "경남 거제시", "경남 거창군", "경남 고성군", "경남 김해시", "경남 남해군",
    "경남 밀양시", "경남 사천시", "경남 산청군", "경남 양산시", "경남 의령군",
    "경남 진주시", "경남 창녕군", "경남 창원시 마산합포구", "경남 창원시 마산회원구",
    "경남 창원시 성산구", "경남 창원시 의창구", "경남 창원시 진해구",
    "경남 통영시", "경남 하동군", "경남 함안군", "경남 함양군", "경남 합천군",
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-5">
      <div className="bg-white w-full max-w-[353px] rounded-2xl overflow-hidden flex flex-col max-h-[80vh]">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center gap-2">
            <MapPin className="size-5 text-purple-600" />
            <h2 className="font-bold text-lg">내 동네 설정</h2>
          </div>
          <button
            onClick={onClose}
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-xl transition-colors ${
                selectedLocation === location
                  ? "bg-purple-50 border-2 border-purple-600 text-purple-700 font-medium"
